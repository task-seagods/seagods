...
if ($loggedin = logged_inadmin()) { // Check if they are logged in
    ...
    while ($row_category = mysql_fetch_array($sql_caregory)) {
        ...
        while ($row_sub_category = mysql_fetch_array($sql_subcaregory)) {
            $content .= '<option value="' . $row_category["id_cat"] . "-" . $row_sub_category["id_cat"] . '" ' . (isset($_GET['id']) && $data_item["id_cat"] == $row_sub_category["id_cat"] ? "selected" : "") . '>' . $row_sub_category["category"] . '</option>';
        }
    ...
    if (isset($_GET["id"])) {
        ...
        while ($rows = mysql_fetch_array($photo)) {
            $content .= '<img style="height:200px" src="images/product/150/thumb_' . (isset($_GET['id']) ? strip_tags(trim($rows["photo"])) : "") . '"><a href="form_products.php?id_photo=' . (isset($_GET['id']) ? strip_tags(trim($rows["id_photo"])) : "") . '">delete</a>';
        }
    }
